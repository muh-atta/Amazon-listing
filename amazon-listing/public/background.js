          ...
          chrome.scripting.executeScript(
            {
              ...
              func: () => {
                ...
                const title =
                  document.querySelector("h1, #productTitle")?.innerText ||
                  "No title";
                const price =
                  ...
                  "Price not available";
                const rating =
                  document.querySelector(".a-icon-alt")?.innerText ||
                  "No rating";
                const images = Array.from(
                  document.querySelectorAll(
                    "#altImages img, .imgTagWrapper img",
                  ),
                )
                  .map((img) => img.src || img.getAttribute("data-old-hires"))
                  .filter(Boolean);
                const seller =
                  document.querySelector("#sellerProfileTriggerId, #bylineInfo")
                    ?.innerText || "Seller not available";
            ...
            (results) => {
              ...
              //   Setting details in localstorage
